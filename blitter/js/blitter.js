...
const BLITTER_UUID   = "8bc01404-0006-4bf4-95d1-ce27a0477183";
...
const settings = {
	...
	blitter: {
		...
		writeBusy: false,
		writeValue: null,
		// Render the live blitter display into the mirror canvas. The
				...
				requestAnimationFrame(() => {
					...
					try {
						if (self._pendingFrame) {
							renderBlitterFrame(new DataView(self._pendingFrame.buffer));
						}
					} finally {
...
async function onConnected(attemptId) {
	...
	// User-requested settle time: wait an extra 1s after the link is connected
...
// Resolve once a live blitter NOTIFICATION frame has been received (see
// settings.blitter._mirrorLive, set only by the notification handler — NOT by
...
function waitForMirrorLive(timeoutMs) {
	const setting = settings.blitter;
...
// Subscribe the live-mirror blitter stream. Called only AFTER the client is
...
async function startBlitterStream(attemptId) {
	try {
		const setting = settings.blitter;
		...
		if (service) {
			try {
				...
				console.log("could not re-fetch blitter characteristic");
				console.log(error && error.message);
			}
		}
		...
		if (setting.characteristic && setting.characteristic.properties.notify) {
			...
			setting._mirrorHandler = (event) => {
				...
				if (!setting._mirrorLive) console.log("blitter mirror stream live");
			...
			// blitter CCCD (it still believes notifications are on from before
			...
			// shows "blitter notifications enabled". Explicitly stop first so
			...
			for (let attempt = 0; ; attempt++) {
				try {
					...
					console.log("blitter notifications enabled");
					noteGattOp(true);
					break;
				} catch (error) {
					if (error && error.message === SUPERSEDED) throw error;
					if (attempt >= 3) {
						console.log("blitter startNotifications failed");
						console.log(error && error.message);
						noteGattOp(false, "blitter startNotifications");
						throw error;
					}
					await sleep(200);
				}
			...
			console.log("blitter characteristic unavailable for the mirror stream");
		}
	} catch (error) {
		// Let a superseded attempt abort the whole chain (handled by the caller).
		if (error && error.message === SUPERSEDED) throw error;
		console.log("error subscribing to blitter mirror");
		console.log(error && error.message);
	}
}
...
async function setupGatt(device, attemptId) {
	...
	for (const key of settingKeys) {
		...
		if (setting.properties.includes("BLERead") && key !== "blitter") {
			for (let attempt = 0; ; attempt++) {
				try {
		...
		// The blitter (live-mirror) stream is NOT started here — it begins only
		// after onConnected() (fully connected), via startBlitterStream().
		if (key !== "blitter" && setting.characteristic.properties.notify) {
...
function updateBlitter(value) {
	...
	settings.blitter.writeValue = new Uint8Array(value.buffer);
	BLEwriteTo("blitter");
...
// self-contained. Ported from opencode-blitter-plugin/src/dots.ts.
...
// 8x8 mirror canvas showing the blitter's live display (overlay adds the dot grid).
...
function stopAmbience() {
	...
	capture = null;
	// Cancel any queued blitter frame: BLEwriteTo coalesces writes with a
	...
	settings.blitter.writePending = false;
	settings.blitter.writeValue = null;
}
...
// to the blitter characteristic, and firmware 0.3.0-unbonded treats EVERY 256-byte
// blitter write as an ambience frame — flipping the device straight back to
...
async function streamer() {
	...
	try {
		...
		updateBlitter(frameCtx.getImageData(0, 0, 8, 8).data);
	} catch (err) {
