...
function get_data_ack(p)
{
	...
	if (p.dirty){
		save_button.style.backgroundColor = 'lightgreen';
		reload_button.style.backgroundColor = 'lightgreen';
	}
	else{
		save_button.style.backgroundColor = 'gray';
		reload_button.style.backgroundColor = 'gray';
	}
...
function set_data_req()
{
	...
	for (var i=0; i<layerGrid.data.length; i++){
		...
		parameters.layers.data[i].values.alpha    = parseFloat(layerGrid.data[i].columns[4]);
		parameters.layers.data[i].values.xflip    = JSON.parse(layerGrid.data[i].columns[5]);
		parameters.layers.data[i].values.yflip    = JSON.parse(layerGrid.data[i].columns[6]);
	}
...
function project_ondelete()
{
	var params = {};

	//validate filename
	params.project = save_filename.value;
	socket_send(selected_server, "delete_project_req", params);
}
